...
- a non-sensitive internal object ID;
...
internal owner ID and object ID. The storage adapter returns only an advertised
byte length and an asynchronous byte stream.

Before any staging write, the reader:

1. validates internal identities and manifest evidence;
...
abort cleanup itself fails, the original processing error remains authoritative
and the durable worker failure path can retry or quarantine the dispatch.
