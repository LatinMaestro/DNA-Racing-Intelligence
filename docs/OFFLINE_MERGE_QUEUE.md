...
|    75 | Hosted game-rule attestations       | `agent/hosted-game-rules-attestations`             | `faf5bce494900d54c3a217b8545df40f8ee68a49` | PR the ten-commit exact-head confirmed-rule evidence delta after order 74            |
...
### Breeding economic actions

Resolve Clerk identity independently for completed/refunded breeding evidence
and optional offspring cost-basis requests, then delegate only to the validated
owner-scoped write service. The repository remains explicitly unavailable, so
Arena listings, incomplete evidence, duplicate costs and ambiguous refunds
cannot enter accounting. Five focused action tests pass inside the complete
28-file/201-test hosted harness; the dependent one-file formatting fix makes the
...
### Breeding economic forms

Render separate semantic completed/refunded breeding-evidence and offspring
cost-basis forms inside the authenticated Breeding Server Component. Every
fieldset and submit control remains disabled until strict parsing, durable IDs,
action feedback and forced-RLS persistence are connected. Three focused
rendering tests pass inside the complete 30-file/208-test hosted harness; all
three form, test and contract blobs reconcile exactly. Arena listings cannot
...
### Vault Performance strict FormData

Parse staged manual-ledger and unallocated tournament-payout evidence through a
server-configured boundary that rejects unknown or duplicate fields,
browser-supplied durable IDs, ambiguous timestamps, malformed exact amounts,
unconfigured category/subcategory pairs and client-controlled asset metadata.
BGC remains separate and excluded from tournament payouts. Ten focused parser
...
### Hosted migration attestations

Require fixed apply, smoke, reverse and removal commands on one exact head,
reviewed migration-set digest and opaque target fingerprint. The target must be
ephemeral and non-Production; evidence must be redacted and synthetic-only; the
final schema digest must equal the pre-apply baseline. Formatting, ESLint,
strict TypeScript and the complete 46-file/334-test hosted harness pass with
zero production vulnerabilities; all three implementation, test and contract
...
complete assertions, positive/boundary/negative cases and redacted digests are
mandatory; stale, drifting, incomplete or private evidence fails closed.
Formatting, ESLint, strict TypeScript and the complete 56-file/419-test hosted
