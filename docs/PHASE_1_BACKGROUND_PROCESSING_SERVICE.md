...
positive and cannot exceed one hour. Repository-returned identifiers and claim
states are validated and canonicalized before use. Concurrent retry times must
be canonical, strictly future and no more than one hour from the claim, preventing
stale hot-loop or unbounded-delay evidence. Missing, completed and concurrently
leased dispatches never invoke the processor.
...
counts and whether aggregate refresh is required. Result identifiers are
canonicalized, counts must be safe non-negative integers with at least one source
version, and the refresh flag must be a literal Boolean before activation.
