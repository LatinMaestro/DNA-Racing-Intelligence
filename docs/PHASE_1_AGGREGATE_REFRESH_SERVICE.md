...
Repository-returned claim states and identifiers are runtime-validated and
canonicalized. Unsafe identifiers, malformed fingerprints, stale retry times and
retry evidence more than one hour ahead fail closed.
...
The service runtime-validates and canonicalizes prepared identifiers and counts,
then rejects a result whose source-version fingerprint differs from the durable
claim. This prevents a worker from publishing calculations based on another
owner or dataset generation.
...
- A computation, malformed publication response or publication failure records
  retryable evidence and cannot claim completion.
...
- canonical repository identifiers;
- stale or unbounded retry evidence;
- malformed claim and publication states; and
