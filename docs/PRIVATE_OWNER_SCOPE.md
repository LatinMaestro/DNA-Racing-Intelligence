...
- The substitution watch may show at most two qualified incoming alternatives for any one outgoing Core. Incoming Cores must meet the same no-Genesis, at-most-400 Bike ageing-used and elite or genuinely elite-potential owner selection rules; a legal structural swap alone is insufficient. The owner identified Reese Dylan as currently over the ageing-used guardrail, so that Core must not appear as a qualified substitution. Require a verified used-ageing interpretation before comparing the preserved API ageing source value with 400; do not guess whether the source reports used or remaining ageing. Withhold watch recommendations while the whole-population benchmark or verified Bike ageing-used inputs are unavailable.
