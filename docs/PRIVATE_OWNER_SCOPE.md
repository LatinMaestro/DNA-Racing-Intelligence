...
- The recurring imported game-data set is a growing sequential Race Merge series, one Core Details export (historically/legacy named Bike Details) and one Current Arena export across three imported source families. Seven Race Merge exports are currently supplied; future sequential exports must be accepted without changing the product contract, subject to a practical bounded batch limit.
