...
export function createManualTournamentPayout(
  ...
  if (!["crypto", "fiat"].includes(input.assetKind)) {
    throw new Error("Manual tournament payout asset kind is invalid.");
  }
  if (
  ...
  if (!assetCodePattern.test(assetCode) || assetCode === "BGC") {
    throw new Error("Manual tournament payout asset identity is invalid.");
  }
  ...
  return {
    ...
    tournamentId: requiredTrimmed(input.tournamentId, "Tournament ID"),
    ...
    assetDecimalPlaces: input.assetDecimalPlaces,
