...
export type ManualMaidenOverride = Readonly<{
  overrideId: string;
  coreId: string;
  maidenState: Exclude<MaidenState, "invalid">;
  effectiveAt: string;
  reason: string;
}>;
...
function assertOwnershipEdits(
  edits: readonly ManualOwnershipEdit[],
): void {
  assertUniqueIds(
...
export function buildCurrentVaultRegistry(input: {
  ...
  const active = new Map<
    string,
    Omit<CurrentVaultCore, "profileStatus">
  >();
  ...
  for (const edit of chronological(input.ownershipEdits, ({ editId }) => editId)) {
    if (
    ...
    if (edit.action === "add") {
      ...
      const priorMaidenState = snapshotMaidenState.get(edit.coreId) ?? "unknown";
  ...
  for (const override of chronological(
    input.maidenOverrides,
