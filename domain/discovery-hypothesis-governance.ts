...
export function discoveryHypothesisIsEligible(
  input: Readonly<{
  ...
  if (
    !Number.isSafeInteger(input.supportingRaceCount) ||
    input.supportingRaceCount < 0
  ) {
    throw new Error(
      "Discovery supporting race count must be a non-negative safe integer.",
    );
  }
