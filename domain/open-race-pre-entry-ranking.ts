...
function rejectCandidateCurrentRaceStars(
  candidate: OpenRaceRankedCandidate,
): void {
  const forbiddenKey = Object.keys(candidate).find(
    (key) => key !== "historicalStars" && /(gold|blue|star)/i.test(key),
  );
  if (forbiddenKey !== undefined) {
    throw new Error(
      `Candidate ${candidate.coreId} cannot contain current-race star input.`,
    );
  }
}
...
export function rankOpenRacePreEntry(
  ...
  for (const candidate of input.candidates) {
    rejectCandidateCurrentRaceStars(candidate);
  ...
  const rankedCandidates = sortedCandidates.map((candidate, index) => {
    ...
    const rank =
      leader !== undefined &&
      candidate.profile.medianTimeMs - leader.profile.medianTimeMs <=
        input.materialGapMs
        ? 1
        : index + 1;
    return {
