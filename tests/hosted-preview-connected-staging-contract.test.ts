...
function stagedRow(file: ProbeFile): {
  ...
  if (header === undefined || data === undefined || extra.length !== 0) {
    throw new Error(
      `Synthetic staging probe ${file.clientFileId} is malformed`,
    );
  }
  ...
  if (schema.status !== "ready") {
    throw new Error(
      `Synthetic staging schema is not ready for ${file.clientFileId}`,
    );
  }
  const adapted = adaptSourceRow(schema, data.split(","));
  if (adapted.status !== "ready" || adapted.record === null) {
    throw new Error(
      `Synthetic staging row is not ready for ${file.clientFileId}`,
    );
  }
  const naturalKey =
...
describeConnected("hosted Preview least-privilege staging contract", () => {
  it("stages the current nine-file synthetic contract directly under the runtime role", async () => {
    ...
    try {
      ...
      for (const claimedFile of claim.files) {
        ...
        if (file === undefined) {
          throw new Error(
            `Staging probe cannot map ${claimedFile.uploadFileId}`,
          );
        }
