...
function fixture(input: { cleanupLeavesResidue?: boolean } = {}) {
  ...
  const scenario = createDnaOpenLabP5ConcurrentCheckpointScenario({
    ownerId: "owner-vault@example.test",
    bucketName: "dna-racing-import-preview",
    cycleId: "75000000-0000-4000-8000-000000000390",
    attemptedAt: "2026-08-31T05:10:00.000Z",
    storage,
...
describe("DNA Open Lab P5 concurrent checkpoint provider scenario", () => {
  it("accepts exactly one writer and preserves immutable evidence", async () => {
    ...
    expect(evidence.servingGenerationBefore).toBe(
      evidence.servingGenerationAfter,
    );
    ...
    const runner = createDnaOpenLabP5ComponentRecoveryCaseRunner({
      scenarios: {
        concurrent_checkpoint_advancement: async () => evidence,
      } as never,
    });
    await expect(
      runner("concurrent_checkpoint_advancement"),
  ...
  it("fails closed when temporary evidence cleanup leaves residue", async () => {
    ...
    await expect(scenario()).rejects.toThrow(
      "DNA Open Lab P5 concurrent checkpoint scenario failed.",
    );
