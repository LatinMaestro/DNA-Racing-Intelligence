...
describe("import confirmation owner action", () => {
  it("sanitizes an authentication-provider failure before provider access", async () => {
    const capabilities = readyCapabilities();
    const actionDependencies = dependencies(capabilities.value);

    await expect(
      confirmOwnerDataUpdate(input(), {
        ...actionDependencies,
