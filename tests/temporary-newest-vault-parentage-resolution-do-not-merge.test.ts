...
d('TEMPORARY newest vault parentage resolution - DO NOT MERGE',()=>{it('reads newest child parentage',async()=>{
 ...
 const newest=vault.map(r=>({hid:hid(r.hid),name:String(r.name??''),gender:String(r.gender??''),element:String(r.element??''),type:String(r.type??''),fno:r.fno})).filter(r=>r.hid!==null).sort((a,b)=>(b.hid as number)-(a.hid as number)).slice(0,20);
