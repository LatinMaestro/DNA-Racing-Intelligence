...
describeConnected("offspring breeder history analysis", () => {
  it(
    ...
    async () => {
      ...
      const historyDir = process.env.BREEDING_HISTORY_DIR ?? "artifacts/history";
      const inventory = JSON.parse(await readFile(inventoryPath, "utf8")) as AnyRecord;
      const evaluationCutoff = canonicalTimestamp(inventory.fetchedAt) ?? new Date().toISOString();

      const historyFiles = (await readdir(historyDir))
        .filter((name) => /^breeding-race-history-\d+\.json$/u.test(name))
        .sort();
      if (historyFiles.length === 0) throw new Error("No history shard artifacts found.");
      const history: HistoryRecord[] = [];
      const historyShardSummaries: AnyRecord[] = [];
      for (const file of historyFiles) {
        const payload = JSON.parse(await readFile(`${historyDir}/${file}`, "utf8")) as AnyRecord;
      ...
      const infoByHid = new Map<number, AnyRecord>(
        ((inventory?.coreFamilies?.info ?? []) as AnyRecord[]).map((row) => [Number(row.hid), row]),
      );
      const splicingByHid = new Map<number, AnyRecord>(
        ((inventory?.coreFamilies?.splicing ?? []) as AnyRecord[]).map((row) => [Number(row.hid), row]),
      );
      const directParentsByChild = new Map<number, number[]>(
        Object.entries(inventory?.universe?.directParentsByChild ?? {}).map(([hid, parents]) => [
          Number(hid),
          (parents as unknown[]).map(Number).filter((value) => Number.isSafeInteger(value) && value > 0),
        ]),
      );
      const explicitMintedAt = new Map<number, string>(
        Object.entries(inventory?.spliceDiscovery?.mintedAtByHid ?? {})
          .map(([hid, value]) => [Number(hid), canonicalTimestamp(value)] as const)
          .filter((entry): entry is readonly [number, string] => Number.isSafeInteger(entry[0]) && entry[1] !== null),
      );
      ...
      for (const record of history) {
        ...
        if (!existingFirst || record.startTime < existingFirst) firstRaceByHid.set(record.hid, record.startTime);
      ...
      for (const [key, rows] of byCoreModeDistance) {
        ...
        const speeds = rows.map((row) => row.speedMetresPerSecond).filter(Number.isFinite);
      ...
      for (const rows of rawByScope.values()) {
        ...
        for (const row of rows) {
          fullSummaries.push({ ...row, qualityPercentile: percentileRank(values, row.rawScore) });
        }
      }
      const fullSummaryByKey = new Map(
        fullSummaries.map((summary) => [`${summary.hid}|${summary.mode}|${summary.distanceMetres}`, summary]),
      );
      ...
      for (const rows of indexedHistory.values()) rows.sort((a, b) => (a.startTime ?? "").localeCompare(b.startTime ?? ""));

      const historicalQualityCache = new Map<string, Map<number, { percentile: number; sampleSize: number }>>();
      const historicalQuality = (
        ...
        const rows = (indexedHistory.get(`${mode}|${distanceMetres}`) ?? []).filter(
        ...
        const map = new Map<number, { percentile: number; sampleSize: number }>();
        if (benchmark.length >= MIN_BENCHMARK_CORES) {
          for (const entry of scores) {
            map.set(entry.hid, { percentile: percentileRank(benchmark, entry.score), sampleSize: entry.sampleSize });
          }
        }
      ...
      const relations = [...directParentsByChild.entries()]
        .map(([child, parents]) => ({ child, parents: [...new Set(parents)].filter((parent) => parent !== child) }))
      ...
      for (const relation of relations) {
        ...
        if (!createdAt) {
          skippedRelations.push({ child: relation.child, parents: relation.parents, reason: "no_creation_or_first_race_time" });
          continue;
        }
        const creationAuthority: DraftOutcome["creationAuthority"] = mintedAt ? "minted_at" : "first_race_proxy";
        const modelCutoff = monthStart(createdAt);
        const childSummaries = fullSummaries.filter((summary) => summary.hid === relation.child);
        if (childSummaries.length === 0) {
          skippedRelations.push({ child: relation.child, parents: relation.parents, reason: "no_qualified_offspring_performance" });
          continue;
        }

        for (const childSummary of childSummaries) {
          const benchmark = rawByScope.get(`${childSummary.mode}|${childSummary.distanceMetres}`) ?? [];
          if (benchmark.length < MIN_BENCHMARK_CORES) continue;
          const historical = historicalQuality(childSummary.mode, childSummary.distanceMetres, modelCutoff);
          ...
          const common = {
            ...
            scope: { mode: childSummary.mode, distanceMetres: childSummary.distanceMetres } satisfies BreederScope,
          ...
          drafts.push({ ...common, parentCoreId: String(parentA), coParentCoreId: String(parentB) });
          drafts.push({ ...common, parentCoreId: String(parentB), coParentCoreId: String(parentA) });
          pairLiftRows.push({
        ...
        for (const mode of ["bike", "car", "horse"] as const) {
          const best = childSummaries
            ...
            .sort((a, b) => b.qualityPercentile - a.qualityPercentile || b.sampleSize - a.sampleSize)[0];
          if (!best) continue;
          const historical = historicalQuality(mode, best.distanceMetres, modelCutoff);
          const qualityA = historical.get(parentA);
          const qualityB = historical.get(parentB);
          if (!qualityA || !qualityB) continue;
          const expected = (qualityA.percentile + qualityB.percentile) / 2;
          ...
          const common = {
            ...
            benchmarkPopulationSize: rawByScope.get(`${mode}|${best.distanceMetres}`)?.length ?? 0,
            offspringCreatedAt: createdAt,
            expectedModelCutoff: modelCutoff,
            evaluationCutoff,
            creationAuthority,
      ...
      for (const rows of draftScopes.values()) {
        ...
        for (const row of rows) {
          ...
          if (row.creationAuthority === "minted_at") authoritativeOutcomes.push(outcome);
          else proxyOutcomes.push(outcome);
        }
      }

      const benchmarkResults: AnyRecord[] = [];
      const scopeKeys = [...new Set(authoritativeOutcomes.map((outcome) => scopeKey(outcome.scope)))].sort();
      for (const key of scopeKeys) {
        ...
        const outcomes = authoritativeOutcomes.filter((outcome) => scopeKey(outcome.scope) === key);
        const parentCount = new Set(outcomes.map((outcome) => outcome.parentCoreId)).size;
        ...
        benchmarkResults.push({
          ...
          populationExceptionalOffspringRate: benchmark.populationExceptionalOffspringRate,
          targets: benchmark.assessments.filter((assessment) => assessment.status === "target").slice(0, 50),
          watches: benchmark.assessments.filter((assessment) => assessment.status === "watch").slice(0, 50),
      ...
      for (const summary of fullSummaries) {
        ...
        ownBestRacerQuality.set(key, Math.max(ownBestRacerQuality.get(key) ?? 0, summary.qualityPercentile));
      }
      const averageRacerEliteBreeders: AnyRecord[] = [];
      for (const result of benchmarkResults) {
        for (const target of result.targets as AnyRecord[]) {
          const ownQuality = ownBestRacerQuality.get(`${target.parentCoreId}|${result.scope.mode}`) ?? null;
          if (ownQuality !== null && ownQuality < 90) {
            averageRacerEliteBreeders.push({
              ...
              parentName: infoByHid.get(Number(target.parentCoreId))?.name ?? null,
      ...
      for (const row of pairLiftRows.filter((entry) => entry.creationAuthority === "minted_at")) {
        const pair = [row.parentA, row.parentB].sort((a: number, b: number) => a - b);
      ...
      const pairSynergies = [...pairGroups.entries()]
        .map(([key, rows]) => {
          ...
          return {
            ...
            positiveLiftRate: lifts.filter((lift) => lift > 0).length / lifts.length,
        ...
        .sort((a, b) => b.medianLift - a.medianLift || b.offspringCount - a.offspringCount)
        .slice(0, 200);

      const creationCoverage = {
        lineageChildren: relations.length,
        authoritativeMintedAtChildren: relations.filter((relation) => explicitMintedAt.has(relation.child)).length,
        firstRaceProxyChildren: relations.filter(
          (relation) => !explicitMintedAt.has(relation.child) && firstRaceByHid.has(relation.child),
        ).length,
      };
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          averageRacerEliteBreeders: averageRacerEliteBreeders.sort(
            (a, b) => (b.breederAssessment?.breederScore ?? 0) - (a.breederAssessment?.breederScore ?? 0),
          ),
          pairSynergies,
          method: {
            exactPerformance: "0.8 * median speed + 0.2 * 80th-percentile speed, then population percentile",
            expectedBaseline: "mean of both parents' pre-cutoff exact-distance quality percentiles",
            chronologicalCutoff: "UTC month start at or before minted_at; first-race proxy is exploratory only",
            authoritativeBreederTargets: "minted_at outcomes only",
            modeWide: "offspring's best supported exact-distance specialty within the mode",
          },
        }),
        "utf8",
      );
