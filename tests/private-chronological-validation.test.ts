...
function input(
  ...
  return {
    ...
    historicalStarHoldoutCases: 80_000,
...
describe("Phase 9 private chronological validation evidence", () => {
  ...
  it("requires historical-star evidence to bind its own holdout coverage", () => {
    const result = assessPrivateChronologicalValidation(
      input({
        historicalStarHoldoutCases: 0,
        historicalStarBrierImprovementMillionths: 1_000,
      }),
    );
    expect(result.historicalStarStatus).toBe("insufficient_evidence");
  });
  ...
  it("rejects malformed runtime shapes, non-Boolean gates and unsafe sums", () => {
    expect(() => assessPrivateChronologicalValidation(null as never)).toThrow(
      /object/,
    );
    expect(() =>
      assessPrivateChronologicalValidation(
        input({ externallyChronologicallyOrdered: "true" as never }),
      ),
    ).toThrow(/Boolean/);
    expect(() =>
      assessPrivateChronologicalValidation(input({ capacity: null as never })),
    ).toThrow(/object/);
    expect(() =>
      assessPrivateChronologicalValidation(
        input({
          sourceRows: Number.MAX_SAFE_INTEGER,
          uniqueRows: Number.MAX_SAFE_INTEGER,
  ...
  it("requires canonical evidence identity and signed integer millionths", () => {
    expect(() =>
      assessPrivateChronologicalValidation(input({ evidenceId: " evidence " })),
    ).toThrow(/canonical/);
    expect(() =>
      assessPrivateChronologicalValidation(
        input({ directHistoryBrierImprovementMillionths: 0.5 }),
      ),
    ).toThrow(/signed safe integer/);
  });
});
