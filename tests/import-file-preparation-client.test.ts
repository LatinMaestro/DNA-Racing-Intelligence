...
describe("import file preparation client", () => {
  ...
  it("rejects the retired Current Vault source and an unbounded file set before reading", async () => {
    const hash = hasher();
    const createSha256 = vi.fn(() => hash.value);
    ...
    await expect(
      prepareImportUploadFiles({
        selections: Array.from({ length: 33 }, (_, index) =>
          selection({
    ...
    ).rejects.toThrow("between 1 and 32 selections");
    expect(createSha256).not.toHaveBeenCalled();
  });
