...
describe("Open Race pre-entry ranking", () => {
  ...
  it("does not chain material ties beyond the leader threshold", () => {
    const result = rankOpenRacePreEntry(
      input({
        materialGapMs: 50,
        candidates: [
          candidate("leader", 10_000),
          candidate("near", 10_040),
          candidate("outside", 10_080),
        ],
      }),
    );
    expect(result.rankedCandidates.map(({ rank }) => rank)).toEqual([1, 1, 3]);
  });
  ...
  it("rejects hidden current-race star input nested in a candidate", () => {
    expect(() =>
      rankOpenRacePreEntry(
        input({
          candidates: [
            {
              ...candidate("fast", 10_000),
              currentGoldStar: true,
            } as OpenRaceRankedCandidate,
          ],
        }),
      ),
    ).toThrow("cannot contain current-race star input");
