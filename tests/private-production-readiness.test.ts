...
describe("Phase 9 private Production readiness", () => {
  ...
  it("keeps ageing or stale evidence review-required", () => {
    for (const evidenceFreshness of ["ageing", "stale"] as const) {
      const readiness = assessPrivateProductionReadiness(
        input({ evidenceFreshness }),
      );
      expect(readiness.status).toBe("review_required");
      expect(
        readiness.checks.find(({ code }) => code === "EVIDENCE_FRESHNESS"),
      ).toMatchObject({ status: "review" });
    }
  });
