...
const pairs = Object.freeze([
  { label: "Bright Lights x Overzealous", fatherCoreId: 17053, motherCoreId: 3437 },
  { label: "Dough x Overzealous", fatherCoreId: 2799, motherCoreId: 3437 },
  { label: "Aerosphere x Lustre", fatherCoreId: 1743, motherCoreId: 2020 },
  { label: "Stormstride x Lustre", fatherCoreId: 20321, motherCoreId: 2020 },
]);
...
describeConnected("specialist short-distance breeding pair analysis", () => {
  it("retrieves deterministic offspring descriptors read-only", async () => {
    ...
    await writeFile("artifacts/owner-breeding-pairs.json", JSON.stringify({ schemaVersion: 3, fetchedAt: new Date().toISOString(), results }), "utf8");
