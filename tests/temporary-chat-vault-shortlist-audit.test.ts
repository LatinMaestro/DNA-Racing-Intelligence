...
const enabled = process.env.VAULT_SHORTLIST_AUDIT === "1";
const apiKey = process.env.DNA_OPEN_LAB_API_KEY_1 ?? "";
const TARGET_IDS = [25657, 23484, 23394, 9166] as const;
...
function numberIds(v:unknown):number[]{
  return Array.isArray(v)?v.filter((x):x is number=>typeof x==="number"&&Number.isSafeInteger(x)&&x>0):[];
}

describe.runIf(enabled)("temporary shortlist opponent audit",()=>{
  it("captures four challenger histories and opposition strength",async()=>{
    ...
    async function paced<T>(fn:()=>Promise<T>):Promise<T>{
      const now=Date.now();const delay=Math.max(0,2200-(now-last));
    ...
    for(const doc of raceDocs) for(const hid of numberIds(doc.hids)) participantIds.add(hid);
    ...
    const payload={generatedAt:new Date().toISOString(),targetIds:TARGET_IDS,histories,raceDocs,coreInfo,racingStats};
    await writeFile("vault-shortlist-audit.json",JSON.stringify(payload,null,2),"utf8");
