...
function readyPort() {
  ...
  const createPresignedPut = vi.fn(
    async (input: {
      ...
      url: `https://${ACCOUNT_ID}.r2.cloudflarestorage.com/${input.bucketName}/${input.key}?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Expires=300&X-Amz-SignedHeaders=content-type%3Bhost&X-Amz-Signature=${"c".repeat(64)}`,
    }),
  );
...
describe("Cloudflare R2 private import object storage", () => {
  it("initializes lazily and verifies private-bucket evidence once", async () => {
    ...
    await ready.storage.inspectObject({
      ownerId: OWNER_ID,
      objectId: "file-1",
    });
