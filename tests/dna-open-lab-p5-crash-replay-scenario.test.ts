...
function fixture(input: { cleanupLeavesResidue?: boolean } = {}) {
  ...
  const scenario = createDnaOpenLabP5CrashAfterEvidenceWriteScenario({
    ...
    cycleId: "75000000-0000-4000-8000-000000000099",
    attemptedAt: "2026-08-31T04:30:00.000Z",
    storage,
...
describe("DNA Open Lab P5 crash/replay provider scenario", () => {
  it("uses the production runner and private immutable evidence boundary", async () => {
    ...
    expect(evidence).toMatchObject({
      ...
      checkpointRevisionBefore: 1,
      checkpointRevisionAfter: 2,
      objectCountBefore: 1,
      objectCountAfter: 1,
    });
    expect(evidence.firstReceiptSha256).toBe(evidence.replayReceiptSha256);
    ...
    const runner = createDnaOpenLabP5ComponentRecoveryCaseRunner({
      scenarios: {
        crash_after_evidence_write: async () => evidence,
      } as never,
    });
  ...
  it("fails closed when mandatory temporary evidence cleanup leaves residue", async () => {
    const { scenario, storage } = fixture({ cleanupLeavesResidue: true });
    await expect(scenario()).rejects.toThrow(
      "DNA Open Lab P5 crash replay scenario failed.",
    );
