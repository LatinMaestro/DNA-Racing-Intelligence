...
describe("DNA Open Lab P5 missing/conflicting evidence scenario", () => {
  it("rejects both evidence faults before Neon staging and preserves last-good", async () => {
    ...
    expect(evidence.expectedReceiptSha256).not.toBe(
    ...
    const runner = createDnaOpenLabP5ComponentRecoveryCaseRunner({
      scenarios: {
        missing_or_conflicting_evidence: async () => evidence,
      } as never,
    });
    await expect(
      runner("missing_or_conflicting_evidence"),
