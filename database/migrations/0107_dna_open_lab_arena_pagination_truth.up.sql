    ...
    IF NOT EXISTS (
      ...
      FROM (
        SELECT (page ->> 'page')::integer AS page_number,
          ...
          jsonb_array_length(page -> 'canonical' -> 'listings')
            AS listing_count,
          (page -> 'canonical' ->> 'pageSizeLimit')::integer
            AS page_size_limit,
      ...
      WHERE chain.page_count <> chain.maximum_page
         OR (chain.has_more OR chain.listing_count = chain.page_size_limit)
           IS DISTINCT FROM (chain.page_number < chain.maximum_page)
    ) THEN
