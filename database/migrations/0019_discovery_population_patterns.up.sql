...
BEGIN
  ...
  population_profiles AS (
    SELECT
      owned.owned_source_core_id,
      owned.owned_display_name,
      owned.owned_me_eligible,
      'population_pattern'::text AS relationship,
      7 AS relation_rank,
      profile.mode,
      profile.distance,
      profile.race_count,
      profile.data_current_through
    FROM owned
    JOIN dna.active_core_details owned_details
      ON owned_details.owner_id = owned.owner_id
      AND owned_details.id = owned.owned_core_id
    JOIN dna.active_core_details peer
      ON peer.owner_id = owned_details.owner_id
      AND peer.id <> owned.owned_core_id
      AND peer.core_class = owned_details.core_class
      AND peer.element = owned_details.element
      AND peer.f_number = owned_details.f_number
    JOIN dna.core_performance_profile profile
      ON profile.owner_id = peer.owner_id
      AND profile.source_core_id = peer.source_core_id
    WHERE NOT EXISTS (
      SELECT 1
      FROM dna.core_performance_profile direct_profile
      WHERE direct_profile.owner_id = owned.owner_id
  ...
  evidence_profiles AS (
    SELECT * FROM lineage_profiles
    UNION ALL
    SELECT * FROM population_profiles
  ),
  ranked AS (
    SELECT
      evidence_profiles.*,
