...
ALTER TABLE dna.dna_core_race_history_generation
  ADD COLUMN entrant_mismatch_omission_count integer NOT NULL DEFAULT 0
    CHECK (entrant_mismatch_omission_count BETWEEN 0 AND 500000);
...
ALTER TABLE dna.dna_core_race_history_generation
  ADD CONSTRAINT dna_core_race_history_generation_result_coverage_check CHECK (
    input_result_count = observation_count + replay_duplicate_count
      + entrant_authority_omission_count + entrant_mismatch_omission_count
  );

CREATE FUNCTION dna.begin_dna_core_race_history_generation_v3(
  p_owner_id uuid,
  p_worker_id text,
  p_generation jsonb
)
...
BEGIN
  ...
  IF p_worker_id IS NULL OR p_worker_id <> btrim(p_worker_id)
     ...
     OR (SELECT count(*) FROM jsonb_object_keys(p_generation)) <> 19
     OR NOT (p_generation ?& ARRAY[
       ...
       'entrantMismatchOmissionCount', 'exactDistanceConfirmedCount',
       'acceptedPublishedCellCount', 'missingFormatCount',
       'unsupportedFormatCount', 'unpublishedCellCount', 'observationCount'
     ])
     OR jsonb_typeof(p_generation -> 'entrantMismatchOmissionCount') <> 'number'
     OR p_generation ->> 'entrantMismatchOmissionCount' !~ '^[0-9]+$'
     OR (p_generation ->> 'entrantMismatchOmissionCount')::numeric > 500000
    ...
    RAISE EXCEPTION 'Core history generation v3 authority is invalid';
  END IF;

  v_input_result_count := (p_generation ->> 'inputResultCount')::integer;
  v_mismatch_count :=
    (p_generation ->> 'entrantMismatchOmissionCount')::integer;
  IF v_input_result_count < v_mismatch_count THEN
    RAISE EXCEPTION 'Core history generation v3 counts disagree';
  END IF;
  v_generation_id := (p_generation ->> 'generationId')::character(64);
  v_v2 := jsonb_set(
    p_generation - 'entrantMismatchOmissionCount',
    '{inputResultCount}',
    to_jsonb(v_input_result_count - v_mismatch_count),
    false
  );
  ...
  IF FOUND THEN
    IF v_existing.worker_id <> p_worker_id
       ...
       OR v_existing.entrant_authority_omission_count <>
         (p_generation ->> 'entrantAuthorityOmissionCount')::integer
       OR v_existing.entrant_mismatch_omission_count <> v_mismatch_count
      ...
      RAISE EXCEPTION 'Core history generation v3 replay conflicts';
    END IF;
    RETURN v_existing.state;
  END IF;

  SELECT dna.begin_dna_core_race_history_generation_v2(
    p_owner_id, p_worker_id, v_v2
  ) INTO v_disposition;
  UPDATE dna.dna_core_race_history_generation stored
  SET input_result_count = v_input_result_count,
      entrant_mismatch_omission_count = v_mismatch_count
  WHERE stored.owner_id = p_owner_id AND stored.generation_id = v_generation_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Core history generation v3 begin was not persisted';
  END IF;
  RETURN v_disposition;
END
$function$;

REVOKE ALL ON FUNCTION dna.begin_dna_core_race_history_generation_v3(uuid,text,jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION dna.begin_dna_core_race_history_generation_v2(uuid,text,jsonb) FROM dna_app_runtime;
GRANT EXECUTE ON FUNCTION dna.begin_dna_core_race_history_generation_v3(uuid,text,jsonb) TO dna_app_runtime;
