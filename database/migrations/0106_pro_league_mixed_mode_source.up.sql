...
BEGIN
  ...
  IF p_generation_id IS NULL
     ...
     OR p_non_bike_entry_count NOT BETWEEN 0 AND p_input_observation_count
  ...
  IF NOT EXISTS (
    ...
    JOIN LATERAL (
      SELECT
        count(*) FILTER (
          WHERE row.payload ->> 'mode' IS DISTINCT FROM 'bike'
        )::bigint AS non_bike_entry_count,
        count(*) FILTER (
          WHERE row.payload ->> 'mode' = 'bike'
            AND row.payload ->> 'publishedCellStatus' = 'accepted'
        )::bigint AS accepted_entry_count,
        count(*) FILTER (
          WHERE row.payload ->> 'mode' = 'bike'
            AND row.payload ->> 'publishedCellStatus' = 'missing_format'
        )::bigint AS missing_format_entry_count,
        count(*) FILTER (
          WHERE row.payload ->> 'mode' = 'bike'
            AND row.payload ->> 'publishedCellStatus' = 'unsupported_format'
        )::bigint AS unsupported_format_entry_count,
        count(*) FILTER (
          WHERE row.payload ->> 'mode' = 'bike'
            AND row.payload ->> 'publishedCellStatus' = 'unpublished_cell'
        )::bigint AS unpublished_cell_entry_count
      FROM dna.dna_core_race_history_generation_row row
      WHERE row.owner_id = source.owner_id
    ...
    WHERE active.owner_id = p_owner_id
      ...
      AND coverage.accepted_entry_count = p_accepted_entry_count
      AND coverage.non_bike_entry_count = p_non_bike_entry_count
      AND coverage.missing_format_entry_count = p_missing_format_entry_count
      AND coverage.unsupported_format_entry_count = p_unsupported_format_entry_count
      AND coverage.unpublished_cell_entry_count = p_unpublished_cell_entry_count
  ) THEN
  ...
  IF p_unbenchmarked_entry_count > v_generation.accepted_entry_count
     ...
     OR NOT EXISTS (
       ...
       WHERE active.owner_id = p_owner_id
         ...
         AND coverage.accepted_entry_count = v_generation.accepted_entry_count
         AND coverage.non_bike_entry_count = v_generation.non_bike_entry_count
         AND coverage.missing_format_entry_count = v_generation.missing_format_entry_count
         AND coverage.unsupported_format_entry_count = v_generation.unsupported_format_entry_count
         AND coverage.unpublished_cell_entry_count = v_generation.unpublished_cell_entry_count
     ) THEN
