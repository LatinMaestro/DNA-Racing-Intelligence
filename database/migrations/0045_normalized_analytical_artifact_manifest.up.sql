...
CREATE TABLE dna.normalized_analytical_artifact (
  ...
  artifact_format text NOT NULL CHECK (artifact_format = 'parquet/v1'),
  ...
  CHECK (
    source_type <> 'race_merge'
    OR ready_row_count = 0
    OR (minimum_event_at IS NOT NULL AND maximum_event_at IS NOT NULL)
  ),
  CHECK (
    (state = 'prepared'
...
BEGIN
  ...
  END IF;
  IF p_source_type NOT IN ('race_merge', 'core_details', 'current_arena') THEN
    RAISE EXCEPTION 'normalized artifact source type is unsupported';
  END IF;
  IF p_prepared_at IS NULL THEN
    RAISE EXCEPTION 'normalized artifact preparation timestamp is required';
  END IF;
  IF p_object_id IS NULL
     OR p_object_id <> btrim(p_object_id)
     OR length(p_object_id) NOT BETWEEN 1 AND 512
     OR p_object_id ~ '[[:cntrl:]]'
     OR position('://' in p_object_id) <> 0 THEN
    RAISE EXCEPTION 'normalized artifact object identity is invalid';
  END IF;
  IF p_content_sha256 IS NULL
     OR p_content_sha256 !~ '^[a-f0-9]{64}$'
     OR p_natural_key_set_sha256 IS NULL
     OR p_natural_key_set_sha256 !~ '^[a-f0-9]{64}$'
     OR p_byte_length IS NULL
     OR p_byte_length <= 0 THEN
    RAISE EXCEPTION 'normalized artifact integrity evidence is invalid';
  END IF;
  IF p_source_row_count IS NULL
     OR p_ready_row_count IS NULL
     OR p_quarantined_row_count IS NULL
     OR p_warning_row_count IS NULL
     OR p_source_row_count < 0
     OR p_ready_row_count < 0
     OR p_quarantined_row_count < 0
     OR p_warning_row_count < 0
     OR p_ready_row_count + p_quarantined_row_count <> p_source_row_count
     OR p_warning_row_count > p_source_row_count THEN
    RAISE EXCEPTION 'normalized artifact row evidence is invalid';
  END IF;
  IF (p_minimum_event_at IS NULL) <> (p_maximum_event_at IS NULL)
     OR (
       p_minimum_event_at IS NOT NULL
       AND p_maximum_event_at < p_minimum_event_at
     )
     OR (
       p_source_type <> 'race_merge'
       AND p_minimum_event_at IS NOT NULL
     )
     OR (
       p_source_type = 'race_merge'
       AND p_ready_row_count > 0
       AND p_minimum_event_at IS NULL
     ) THEN
    RAISE EXCEPTION 'normalized artifact chronology evidence is invalid';
  END IF;
  ...
  END IF;
  IF p_prepared_at < v_batch.uploaded_at THEN
    RAISE EXCEPTION 'normalized artifact preparation cannot predate upload';
  END IF;
  IF p_source_row_count <> v_batch.source_rows THEN
  ...
  IF FOUND THEN
    IF v_existing.source_type <> p_source_type
       ...
       OR v_existing.maximum_event_at IS DISTINCT FROM p_maximum_event_at
       OR v_existing.prepared_at <> p_prepared_at THEN
  ...
  INSERT INTO dna.normalized_analytical_artifact (
    ...
    'parquet/v1', 'cloudflare_r2', p_object_id, p_content_sha256,
...
BEGIN
  ...
  END IF;
  IF p_bound_at < v_artifact.prepared_at OR p_bound_at < v_version.activated_at THEN
    RAISE EXCEPTION 'normalized artifact binding cannot predate preparation or activation';
  END IF;
...
BEGIN
  ...
  SELECT artifact.bound_at INTO v_bound_at
  FROM dna.normalized_analytical_artifact artifact
  WHERE artifact.owner_id = p_owner_id
    AND artifact.dataset_version_id = p_dataset_version_id
    AND artifact.state IN ('bound', 'rolled_back')
  FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'bound normalized analytical artifact is unavailable';
  END IF;
  IF p_rolled_back_at < v_bound_at THEN
    RAISE EXCEPTION 'normalized artifact rollback cannot predate binding';
  END IF;
