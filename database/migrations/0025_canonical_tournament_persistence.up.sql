...
ALTER TABLE dna.tournament_configuration
  ADD COLUMN campaign_action jsonb,
  ADD CONSTRAINT tournament_campaign_action_valid CHECK (
    ...
    OR (
      ...
      AND (
        ...
        OR (
          nullif(
            btrim(campaign_action ->> 'ownerAcknowledgedAt'),
            ''
          ) IS NOT NULL
...
CREATE FUNCTION dna.upsert_complete_tournament_configuration(
  p_owner_id uuid,
...
CREATE FUNCTION dna.list_complete_tournament_configurations(p_owner_id uuid)
...
BEGIN
  IF v_owner_id IS NULL OR p_owner_id <> v_owner_id THEN
    RAISE EXCEPTION 'owner-scoped complete Tournament configuration read denied';
  END IF;
...
GRANT EXECUTE ON FUNCTION dna.upsert_complete_tournament_configuration(
  uuid, text, text, text, timestamptz, timestamptz, text, text, text,
  integer[], integer, numeric, text, text, text[], text[], text[],
  integer[], jsonb, jsonb, text, jsonb, integer, integer, numeric,
  text, integer, jsonb, jsonb, text, text, text, text, text, jsonb,
