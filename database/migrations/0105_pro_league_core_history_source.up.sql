...
ALTER TABLE dna.pro_league_evidence_generation
  ADD COLUMN source_kind text NOT NULL DEFAULT 'race_dataset_version'
    CHECK (source_kind IN ('race_dataset_version', 'core_history_generation')),
  ADD COLUMN core_history_generation_id character(64),
  ALTER COLUMN race_dataset_version_id DROP NOT NULL;

ALTER TABLE dna.pro_league_evidence_generation
  ADD CONSTRAINT pro_league_evidence_core_history_generation_fkey
  FOREIGN KEY (owner_id, core_history_generation_id)
  REFERENCES dna.dna_core_race_history_generation(owner_id, generation_id)
  ON DELETE RESTRICT,
  ADD CONSTRAINT pro_league_evidence_source_identity_check CHECK (
    (source_kind = 'race_dataset_version'
      AND race_dataset_version_id IS NOT NULL
      AND core_history_generation_id IS NULL)
    OR
    (source_kind = 'core_history_generation'
      AND race_dataset_version_id IS NULL
      AND core_history_generation_id IS NOT NULL)
  );

CREATE FUNCTION dna.begin_pro_league_evidence_generation_from_core_history(
  p_owner_id uuid,
...
CREATE FUNCTION dna.publish_pro_league_evidence_generation_from_core_history(
  p_owner_id uuid,
  p_generation_id uuid,
  p_worker_id text,
