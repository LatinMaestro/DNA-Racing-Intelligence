...
DECLARE
  ...
  v_benchmark_min_refreshed_at timestamptz;
  v_benchmark_max_refreshed_at timestamptz;
  v_star_profile_count bigint;
  v_star_profile_min_refreshed_at timestamptz;
  v_star_profile_max_refreshed_at timestamptz;
...
BEGIN
  ...
  SELECT
    count(*),
    min(benchmark.refreshed_at),
    max(benchmark.refreshed_at)
  INTO
    v_benchmark_count,
    v_benchmark_min_refreshed_at,
    v_benchmark_max_refreshed_at
  FROM dna.discovery_exact_distance_benchmark benchmark
  WHERE benchmark.owner_id = p_owner_id;

  IF v_benchmark_count = 0
    OR v_benchmark_min_refreshed_at <> v_race_version.aggregate_refreshed_at
    OR v_benchmark_max_refreshed_at <> v_race_version.aggregate_refreshed_at
  THEN
    RETURN 'snapshot-unbound';
  END IF;
  ...
  INTO
    ...
    v_star_profile_min_refreshed_at,
    v_star_profile_max_refreshed_at
  FROM dna.core_star_profile profile
  WHERE profile.owner_id = p_owner_id;

  IF v_star_profile_count = 0
    OR v_star_profile_min_refreshed_at <> v_race_version.aggregate_refreshed_at
    OR v_star_profile_max_refreshed_at <> v_race_version.aggregate_refreshed_at
  THEN
    RETURN 'snapshot-unbound';
  END IF;
  ...
  v_snapshot_payload := jsonb_build_object(
    ...
    'benchmarkCount', v_benchmark_count,
    'benchmarkRefreshedAt', to_char(
      timezone('UTC', v_benchmark_min_refreshed_at),
      'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'
    ),
    'starProfileCount', v_star_profile_count,
    'starProfileRefreshedAt', to_char(
      timezone('UTC', v_star_profile_min_refreshed_at),
      'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'
    ),
