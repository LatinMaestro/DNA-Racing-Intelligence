...
CREATE OR REPLACE FUNCTION dna.publish_dna_open_lab_daily_refresh_generation(
  p_owner_id uuid,
    ...
    IF v_finished.previous_published_cycle_id IS NULL
       OR NOT EXISTS (
         WITH RECURSIVE lineage AS (
           SELECT publication.cycle_id, publication.previous_published_cycle_id
           FROM dna.dna_open_lab_finished_race_incremental_publication publication
           WHERE publication.owner_id = p_owner_id
             AND publication.cycle_id = v_finished.previous_published_cycle_id
           UNION
           SELECT parent.cycle_id, parent.previous_published_cycle_id
           FROM lineage child
           JOIN dna.dna_open_lab_finished_race_incremental_publication parent
             ON parent.owner_id = p_owner_id
            AND parent.cycle_id = child.previous_published_cycle_id
         )
         SELECT 1
         FROM lineage
         WHERE lineage.cycle_id = v_previous.finished_history_cycle_id
       )
