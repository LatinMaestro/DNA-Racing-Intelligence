...
BEGIN
  ...
  SELECT COALESCE(sum(evidence.evidence_partition_count), 0)::bigint
  ...
  IF v_partition_count < 1 OR v_partition_count > 10000 THEN
    RAISE EXCEPTION 'Race archive aggregate total partition count exceeds its bound';
  END IF;
