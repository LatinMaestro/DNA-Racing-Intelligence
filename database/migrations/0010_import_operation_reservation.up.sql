...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'dna_app_runtime'
  ) THEN
    CREATE ROLE dna_app_runtime
      LOGIN
      NOSUPERUSER
      NOCREATEDB
      NOCREATEROLE
  ...
  THEN
    RAISE EXCEPTION 'dna_app_runtime must remain least privileged';
  END IF;
END
$runtime_role$;
...
GRANT USAGE ON SCHEMA dna TO dna_app_runtime;
GRANT SELECT ON TABLE dna.app_owner TO dna_app_runtime;
GRANT SELECT, INSERT ON TABLE dna.import_operation_reservation
  TO dna_app_runtime;
GRANT EXECUTE ON FUNCTION dna.current_owner_id() TO dna_app_runtime;
GRANT EXECUTE ON FUNCTION dna.reserve_import_operation(
  uuid,
  text,
  text,
  character,
