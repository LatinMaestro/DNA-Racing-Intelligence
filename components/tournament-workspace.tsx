...
> = {
  ...
  read_model_connected: {
    ...
    detail:
      "Candidate order follows the configured qualification metric. Imported history is not the current field and does not authorise entry.",
  },
};
    ...
    <div className="space-y-8">
      <header className="max-w-4xl">
        ...
        <p className="mt-4 text-base leading-7 text-[var(--muted)]">
          Review candidates against versioned tournament and bracket rules. The
          50% race gate is a hard cap, not a target, and Maiden eligibility is
          preserved for the strongest projected mode-specific opportunity.
        </p>
      </header>
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <span className="rounded-full border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--muted)]">
                    Configured metric order
                  </span>
                </div>

                {bracket.candidates.length === 0 ? (
                  <p className="mt-5 text-sm leading-6 text-[var(--muted)]">
                    No candidates are available for this bracket.
                  </p>
                ) : (
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-[var(--muted)]">
                              {candidate.reviewOrder === null
                                ? "Not in review order"
                                : `Review ${candidate.reviewOrder}`}
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <dd className="mt-1 font-semibold">
                              {candidate.configuredMetricRank === null
                                ? "Unavailable"
                                : `Rank ${candidate.configuredMetricRank}`}
                            </dd>
                          </div>
                          <div>
                            <dt className="text-[var(--muted)]">
