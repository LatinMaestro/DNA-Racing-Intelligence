...
function normalizeClaim(
  ...
  if (typeof value !== "object" || value === null || !("status" in value)) {
    throw new Error("aggregate refresh claim is invalid");
  }
  ...
  switch (claim.status) {
    ...
    case "already_complete":
      return {
        ...
        aggregateSetId: requireSafeIdentifier(
          claim.aggregateSetId,
          "aggregateSetId",
        ),
      };
    ...
    case "claimed":
      return {
        ...
        sourceVersionSetSha256: requireSha256(
          claim.sourceVersionSetSha256,
          "sourceVersionSetSha256",
        ),
      };
    default:
      throw new Error("aggregate refresh claim status is invalid");
  }
}

function normalizePreparedRefresh(
  value: unknown,
  expectedSourceVersionSetSha256: string,
): PreparedAggregateRefresh {
  if (typeof value !== "object" || value === null) {
    throw new Error("prepared aggregate refresh is invalid");
  }
  const result = value as Record<string, unknown>;
  const sourceVersionSetSha256 = requireSha256(
    result.sourceVersionSetSha256,
    "sourceVersionSetSha256",
  );
  if (sourceVersionSetSha256 !== expectedSourceVersionSetSha256) {
    throw new Error("Prepared aggregate source versions do not match claim.");
  }
  const aggregateFamilyCount = nonNegativeSafeInteger(
    result.aggregateFamilyCount,
    "aggregateFamilyCount",
  );
  if (aggregateFamilyCount === 0) {
    throw new Error("aggregateFamilyCount must be positive");
  }
  return {
    preparedAggregateSetId: requireSafeIdentifier(
      result.preparedAggregateSetId,
      "preparedAggregateSetId",
    ),
    sourceVersionSetSha256,
    aggregateFamilyCount,
    materializedRowCount: nonNegativeSafeInteger(
...
function normalizePublication(
  ...
  | Readonly<{ status: "published"; aggregateSetId: string }>
  | Readonly<{ status: "superseded" }> {
  if (typeof value !== "object" || value === null || !("status" in value)) {
    throw new Error("aggregate publication is invalid");
  }
  const publication = value as Record<string, unknown>;
  if (publication.status === "superseded") return { status: "superseded" };
  ...
  throw new Error("aggregate publication status is invalid");
}
...
export async function runAggregateRefreshDispatch(
  ...
  try {
    prepared = normalizePreparedRefresh(
  ...
  try {
    publication = normalizePublication(
      await repository.publishPreparedAggregateSet({
        ownerId: claim.ownerId,
        updateSessionId: claim.updateSessionId,
        refreshId,
        workerId,
  ...
  return {
    status: "completed",
    updateSessionId: claim.updateSessionId,
    aggregateSetId: publication.aggregateSetId,
    aggregateFamilyCount: prepared.aggregateFamilyCount,
    materializedRowCount: prepared.materializedRowCount,
  };
}
