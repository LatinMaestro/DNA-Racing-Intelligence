...
function parseAmzDate(value: string): number {
  ...
  if (match === null) throw new Error("R2 signing time is invalid.");
  ...
  const canonical = new Date(milliseconds)
    .toISOString()
    .replace(/[-:]|\.000/g, "");
  if (canonical !== value) throw new Error("R2 signing time is invalid.");
  return milliseconds;
}
...
function validatePresignedTarget(input: {
  ...
  const credential = target.searchParams.get("X-Amz-Credential") ?? "";
  ...
  const signedAt = parseAmzDate(target.searchParams.get("X-Amz-Date") ?? "");
  const approvedExpiry = Date.parse(input.expiresAt);
  const signedExpiry = signedAt + expiresIn * 1000;
  if (
    ...
    expiresIn < 1 ||
    expiresIn > 3600 ||
    !Number.isFinite(approvedExpiry) ||
    signedExpiry > approvedExpiry
  ) {
  ...
  return Object.freeze({
    ...
    async inspectObject(inspectInput) {
      ...
      const uploadBatchId = requireSafeIdentifier(
        inspectInput.uploadBatchId,
        "uploadBatchId",
      );
      const uploadFileId = requireSafeIdentifier(
        inspectInput.uploadFileId,
        "uploadFileId",
      );
      const objectId = requireSafeIdentifier(inspectInput.objectId, "objectId");
      if (objectId !== uploadFileId) {
        throw new Error("R2 upload object identity is inconsistent.");
      }
      const created = await privatePort();
